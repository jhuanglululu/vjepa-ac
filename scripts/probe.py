...
def parse_args():
    ...
    p.add_argument("--strides", type=int, nargs="+", default=[1, 2, 4, 8])
    ...
    p.add_argument("--target", choices=["action", "dstate", "state"], default="action")
    p.add_argument("--device", default=None)
    p.add_argument("--out", default="records/diagnostics/probe_results.json")
    return p.parse_args()
...
if args.target == "dstate":
    ...
    raw_targets = lambda idx, k: (actions[idx], cs[idx + k] - cs[idx])  # noqa: E731
    half_names = ("first", "sum")
...
for k in args.strides:
    ...
    stats = (
        t1.mean(0),
        t1.std(0).clamp(min=1e-6),
        t2.mean(0),
        t2.std(0).clamp(min=1e-6),
    )
...
print(
    f"\n=== probe target={args.target}, R2 as {half_names[0]}/{half_names[1]}, "
    f"{n_val_eps} held-out episodes ==="
)
...
for r in rows:
    ...
    print(
        f"{r['stride']:>6} | {r['variant']:>7} | {cells[0]:>13} | {cells[1]:>13} | {cells[2]:>13}"
    )

...
class DecoderBlock(nn.Module):
    def __init__(self, config, rope: RoPE):
        super().__init__()
        ...
        self.norm1 = nn.RMSNorm(d_model, eps)
        self.attn = MultiHeadAttention(d_model, n_heads, rope)
        self.norm2 = nn.RMSNorm(d_model, eps)
        self.mlp = SiLUMlp(d_model, d_ff)
...
class Predictor(nn.Module):
    def __init__(self, config):
        ...
        width, height = config["latent_size"]
        d_model = config["d_model"]
        ...
        rope = RoPE(d_head, max_seq_len)

        self.decoder_stack = nn.ModuleList(
            [DecoderBlock(config, rope) for _ in range(n_layers)]
        )

        self.out_norm = nn.RMSNorm(d_model, eps)
        self.out_proj = nn.Linear(d_model, d_state)

        nn.init.normal_(self.state_embed.weight, std=0.02)
        nn.init.normal_(self.action_embed.weight, std=0.02)
        nn.init.normal_(self.out_proj.weight, std=0.02)

    def forward(self, states: list[Tensor], actions: list[Tensor]) -> Tensor:
        B = states[0].shape[0]
        T = len(states)
        ...
        ts = torch.arange(T, device=s.device).repeat_interleave(P + 1)
